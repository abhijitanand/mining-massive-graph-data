...
public class AllNodeLabelling {
    ...
    public Set<DefaultEdge> run(int loopLimit) {
        ...
        //Track label difference
        HashMap<String, Integer> initLabels = new HashMap<>();
        for (String vertex : inputGraph.vertexSet()) {
            initLabels.put(vertex, 0);
        }
        ...
        for (String incomingBall : inputGraph.vertexSet()) {
            ...
            while (isOccupied(bestBin, matching)) {
                ...
                //update labels 
                change += updateLabels(incomingBall, bestBin, nextBestBin, binLabels, loopLimit);
            ...
            if (bestBin == null || binLabels.get(bestBin) >= loopLimit) {
                if (change < 0) {
                    System.out.println("Change reduces .." + change);
                }
                continue;
            }
...
//            System.out.println("Change : " + change);
    ...
    private int updateLabels(String incomingBall, String bestBin, String nextBestBin,
        HashMap<String, Integer> labels, int looplimit) {
        int nextBestLabel = (nextBestBin == null)
            ? looplimit + 1 : labels.get(nextBestBin) + 2;

        int prev = labels.get(bestBin);
        int change = nextBestLabel - prev;
        if (prev > nextBestLabel) {
            System.out.println("Label decreases : " + prev + " --> " + nextBestLabel);
        }
